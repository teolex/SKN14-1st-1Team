...
# 비교할 모델 선택
selected_models = st.multiselect('비교할 모델을 선택하세요', df['Model'].unique())

if selected_models:
    compare_df = df[df['Model'].isin(selected_models)]

    # 중복 모델 알림
    duplicate_models = compare_df['Model'][compare_df['Model'].duplicated()]
    if not duplicate_models.empty:
        st.warning(f"선택한 모델 중 중복된 모델이 있습니다: {duplicate_models.unique().tolist()}")

    # Model을 인덱스로 설정 후 전치
    transposed_df = compare_df.set_index('Model').T
    ...
    # 실제 존재하는 컬럼만 사용
    spec_cols = [col for col in spec_cols if col in df.columns]

    if spec_cols:
        selected_spec = st.selectbox('비교 항목을 선택하세요', spec_cols)
        fig = px.bar(compare_df, x='Model', y=selected_spec, title=f'모델별 {selected_spec} 비교')
        st.plotly_chart(fig)
    else:
